...
exploreTimeLine.from(descriptionSplit.lines, {
  ...
  return (
    <>
        <section className="relative h-screen py-20 text-center flex flex-col items-center max-md:pb-30 bg-blue-50" id="explore">
            <div className="md:mb-10">
            <span className="small-title text-main-gredient">DISCOVER EXTRAORDINARY PLACES</span>
            <AnimatedTitle title1="Explore the World's" title2="Most Breathtaking Destinations" specialWord="Breathtaking" />
            </div>
            ...
            <div className=" size-200 max-lg:size-190 max-md:size-170 max-sm:size-80 rounded-2xl overflow-hidden" id="explore-clip">
                {imagesData.map((image, index) => (
                    <>
                    <img className={`object-cover object-center min-w-full min-h-full absolute top-0 left-0 rounded-2xl`} style={{ zIndex: (imagesData.length * 10) - (index * 10) }} src={image.src} alt={image.smallTitle} key={index} id={image.id + 'Image'} />
